...
for (i in subs_brefLinks) {
  ...
  # Tables are scraped from the website using its table ID in the html. This is universal across all basketball-reference player pages.
  tempPerGame <- pageobj %>%  
...
#Save both the advanced and per game datasets to the 'Data' folder.

here::here()
save(Advanced, file = file.path("data","Advanced.rda"))
save(PerGame, file = file.path("data","PerGame.rda"))
